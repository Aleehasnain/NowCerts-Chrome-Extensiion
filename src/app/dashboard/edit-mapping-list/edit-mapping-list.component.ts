...
@Component({
  selector: 'app-edit-mapping-list',
  templateUrl: './edit-mapping-list.component.html',
  styleUrls: ['./edit-mapping-list.component.css'],
})
export class EditMappingListComponent implements AfterViewChecked {
  tagsInformation: any;
  ...
  constructor(
    ...
    this.keyToReplace = this.activatedRoute.snapshot.queryParamMap.get('id') || '';
    this.keyInputType = this.activatedRoute.snapshot.queryParamMap.get('type') || '';
    console.log("My  Key the edit page = ", this.keyToReplace);
    console.log("My Key type on the edit page = ", this.keyInputType);
  }
          ...
          forkJoin({
            ...
            ({ mappingResponse, requestResponse }) => {
              ...
              if (!(this.mapkey in this.fieldsMappings)) {
                ...
                const matchedValue = this.fieldsMappings[this.keyToReplace];
                if (matchedValue) {
                  ...
                  if (words[0] === 'insured') {
                    ...
                    const myData = this.insuredList[
                      this.filteredInsuredList.indexOf(this.selectedInsured)
                    ];
                    const data = this.transformFieldNames([myData]);
                    this.titleToEdit = data[0].toUpperCase();
                    ...
                    this.originalFieldName = `policy.${this.policyList[
                      this.filteredPolicyList.indexOf(this.selectedPolicy)
                    ]
                      }.${this.fieldType}`;
                    const myData = this.policyList[
                      this.filteredPolicyList.indexOf(this.selectedPolicy)
                    ];
                    const data = this.transformFieldNames([myData]);
                    this.titleToEdit = data[0].toUpperCase();
                    ...
                    this.originalFieldName = `driver.${this.driverList[
  ...
  getRequestFromServer(url): Observable<any> {
    return this.http
      .getRequest(
        `nowcerts_extension/nowcerts_field_mappings/fetch_mapping?page_url=${url?.split('?')[0]
        }`
      )
      .pipe(
        map((res) => {
  ...
  saveReference() {
    ...
    if (this.originalFieldName) {
      ...
      data.nowcerts_field_mapping.fields_mapping[this.keyToReplace] =
